...
export function buildBook(l2Hex: string, params: BookParams, vault?: VaultParams): Book {
  ...
  const bidSize = bids[0]![1], askSize = asks[asks.length - 1]![1];
  const mid = (bid + ask) / 2;
  const touch = bidSize + askSize;
  const microprice = touch > 0 ? (ask * bidSize + bid * askSize) / touch : mid;
  ...
  return {
    block: l2.block, bid, ask, mid, microprice, bidSize, askSize,
